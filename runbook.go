...
type runbook struct {
	...
	useMap   bool
	stepKeys []string
	wd       string // working directory for curl file uploads
}
...
// NewRunbook creates a new runbook with the specified description.
// If no description is provided, a default one is used.
...
// ParseRunbook parses a runbook from the provided reader.
// It reads the YAML content and converts it into a runbook structure.
...
func (rb *runbook) curlToStep(in ...string) error {
	p, err := curlreq.NewParser(curlreq.WithWorkingDirectory(rb.wd))
	if err != nil {
		return err
	}
