...
`
	...
	// Download OpenAPI3 Spec for GitHub API
	res, err := http.DefaultClient.Get("https://raw.githubusercontent.com/github/rest-api-description/main/descriptions/api.github.com/api.github.com.yaml")
	if err != nil {
		t.Fatal(err)
	}
...
// githubServer returns responses that are similar to GitHub API (only /issues).
// For benchmarking.
func githubServer(t testing.TB) *httptest.Server {
	ts := httpstub.NewServer(t)
	ts.Path("/issues").ResponseString(http.StatusOK, `[
  {
    "id": 1,
